	...
	: QWidget(parent)
{
	...
	connect(this->m_p_AnimationListPlayer, &P_AnimationListPlayer::signal_backRunChanged, this, &P_COAS_StatePart::backRunChanged);		//反向被设置
	...
	// > 图片查看块 - 连接资源切换
	connect(this->m_p_AnimationListEditor, &P_ALE_Editor::signal_cellChanged, this, &P_COAS_StatePart::bitmapChanged);
...
/*-------------------------------------------------
		控件 - 倒放勾选框变化
*/
void P_COAS_StatePart::backRunChanged(bool checked){
...
void P_COAS_StatePart::btn_editTagTank(){
	...
	d.setParamShowingName("状态元 标签");
	d.setParamDescription("标签名不可重复。");
	d.setParamListDescription("你可以在游戏中通过 插件指令 执行标签播放，播放时将找出含指定标签的状态元，然后播放。（具体需要看 插件指令用法）");
...
void P_COAS_StatePart::local_saveCurIndexData(){
	...
	QStringList pic_list = ALE_data.getData_PicList();
	for (int i = 0; i < pic_list.count(); i++){
		state_ptr->gif_src.append(QFileInfo(pic_list.at(i)).completeBaseName());
	}
	//		（资源文件夹，不需赋值）
...
void P_COAS_StatePart::local_loadIndexData(int index){
	...
	// > 标签
	QString label_text;
	label_text.append("参数 [状态元");
	label_text.append(QString::number(state_ptr->id + 1));
	label_text.append("]");
	ui.groupBox_param->setTitle(label_text);
	label_text = "";
	label_text.append("动画帧 [状态元");
	label_text.append(QString::number(state_ptr->id + 1));
	label_text.append("]");
	ui.groupBox_animPlayer->setTitle(label_text);
	label_text = "";
	label_text.append("视图 [状态元");
	label_text.append(QString::number(state_ptr->id + 1));
	label_text.append("]");
