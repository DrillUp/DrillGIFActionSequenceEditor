...
#include "../OperateBoard/S_RmmvOperateBoardConfig.h"
...
C_RmmvProjectData S_RmmvCaller_ActionSeq::callRmmvSelect(){
	...
	S_RmmvProjectDataContainer::getInstance()->modifyData(result);		//打开后，立即记录位置
	return result;
}
/*-------------------------------------------------
		工程 - 保存操作（覆盖到rmmv）
...
void S_RmmvCaller_ActionSeq::saveDataToRmmv(C_RmmvProjectData rmmvProjectData){
	...
	// > 检查插件（强制升级）
	QFileInfo file_COAS = S_RmmvProjectDataContainer::getInstance()->getRmmvFile_Plugin("Drill_CoreOfActionSequence");
	if (file_COAS.exists()){
	...
	// > 备份文件
	if (S_RmmvOperateBoardConfig::getInstance()->isBakEnabled() == true){
		S_TempFileManager::getInstance()->copy_File(plugin_info, plugin_info.absolutePath() + "/plugins_bak.js");
	}
	// > 备份文件夹
	QString img_path = S_RmmvProjectDataContainer::getInstance()->getRmmvDir_Img().absolutePath() + "/Special__actionSeq";
	if (S_RmmvOperateBoardConfig::getInstance()->isBakEnabled() == true){
	...
	QFileInfo pluginScript_info = S_RmmvProjectDataContainer::getInstance()->getRmmvFile_Plugin("Drill_CoreOfActionSequence");
	...
	// ---------------------------------------------
	// > 文件清空
	QString src_path = S_RmmvProjectDataContainer::getInstance()->getRmmvDir_Img().absolutePath() + "/Special__actionSeq";
