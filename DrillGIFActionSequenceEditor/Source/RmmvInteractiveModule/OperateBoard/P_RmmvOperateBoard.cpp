	...
	: QWidget(parent)
{
	...
	connect(ui.checkBox_bakEnabled, &QCheckBox::toggled, this, &P_RmmvOperateBoard::bakChanged);
}
...
/*-------------------------------------------------
		控件 - 备份设置变化时
*/
void P_RmmvOperateBoard::bakChanged(){
	S_RmmvOperateBoardConfig::getInstance()->setBakEnabled(ui.checkBox_bakEnabled->isChecked());
}
...
void P_RmmvOperateBoard::putDataToUi() {
	...
	ui.lineEdit_path->setText(this->local_rmmvData.getRootPath());
	ui.lineEdit_name->setText(this->local_rmmvData.getName());
	ui.checkBox_bakEnabled->setChecked(S_RmmvOperateBoardConfig::getInstance()->isBakEnabled());
}
/*-------------------------------------------------
		窗口 - ui数据 -> 本地数据
*/
